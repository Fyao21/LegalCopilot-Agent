...
def ensure_article_embeddings(db: Session, provider: EmbeddingProvider) -> int:
    ...
    pending = [article for article in articles if article.id not in existing or existing[article.id].content_hash != content_hash(article.content)]
    if not pending:
        return 0
    ...
    for article, vector in zip(pending, vectors):
        ...
        if row is None:
            row = ArticleEmbedding(article_id=article.id, provider=provider.provider_name, model=provider.model_name)
            db.add(row)
