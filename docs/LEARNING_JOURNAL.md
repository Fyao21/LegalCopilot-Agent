...
# 2026-08-01：修复带法规日期的智能报告生成失败

## 1. 解决的问题

智能 Agent 已成功完成案件要素提取、混合检索和引用真实性审核，但进入“生成报告草稿”节点后停在 85%，接口返回：

```text
Object of type date is not JSON serializable
```

第七周引入法规版本后，引用对象包含 `effective_from` 和 `effective_to` 两个 Python `date` 字段。报告写作服务使用普通 `model_dump()` 生成 Python 字典，再用标准库 `json.dumps()` 构造大模型输入。普通模式会保留 `date` 对象，而 JSON 标准只支持字符串、数字、布尔值、数组、对象和 `null`，因此在真正发起大模型请求前就发生异常。

## 2. 修改或新增的关键文件

- `app/services/report_writer.py`
  - 报告初稿的大模型输入改用 `model_dump(mode="json")`；
  - 人工审批后的报告修订输入同步采用 JSON 模式；
  - 案件事实与已审核引用使用同一种序列化边界。
- `tests/test_report_writer.py`
  - 新增带法规生效日期的报告初稿回归测试；
  - 新增带法规生效日期的报告修订回归测试；
  - 使用捕获型假 LLM 检查实际传入模型的 JSON，而不依赖外部 API。

## 3. 核心实现原理

Pydantic 的两种导出模式含义不同：

```python
model.model_dump()
# Python 模式：date(2021, 4, 29) 仍然是 date 对象

model.model_dump(mode="json")
# JSON 模式："2021-04-29"
```

本次选择 `mode="json"`，而不是给 `json.dumps()` 增加笼统的 `default=str`。前者由字段模型负责类型转换，输出格式明确且可以测试；后者可能把意外出现的未知对象也静默转换为字符串，从而掩盖数据建模错误。
...
用户提交案件
  → LangGraph 提取案件事实
  → 按案件日期检索法规版本
  → Citation 保存 effective_from/effective_to
  → 引用真实性审核
  → ReviewedCitation.model_dump(mode="json")
  → 日期转换为 YYYY-MM-DD 或 null
  → json.dumps 构造合法模型请求
  → LLM 生成 ReportDraft
  → 渲染 Markdown 报告
  → 进入人工审批
...
报告被要求修改时，`revise_report_markdown()` 也会携带相同的引用日期，因此必须同时修复初稿和修订两条路径，不能只解决截图中首先出现的初稿异常。

## 5. 测试和实际结果

新增两项回归测试：

1. 报告初稿输入中的 `effective_from` 必须是 `"2021-04-29"`，`effective_to` 必须是 `null`；
2. 报告修订输入也必须满足相同的 JSON 日期格式。
...
- 完整测试：84 / 84 通过；
- Ruff check：通过；
- Mypy：27 个检查范围内的源文件无错误。

## 6. 已知限制

1. 回归测试使用假 LLM 验证本项目发送的 JSON，没有调用真实 DeepSeek 服务；
2. 外部模型仍可能因为超时、限流、认证或结构化输出不符合 Schema 而降级，但不会再因为 Python 日期对象而失败；
3. 历史已经失败的运行不会自动重新执行，需要用户重新提交案件；
4. 前端显示 85% 是节点开始时的进度，异常发生后仍保留该进度；后续可增加更具体的失败节点和错误码展示。

## 7. 面试可能追问与回答要点

### 问题一：为什么日期不能直接放进 JSON？

JSON 规范没有日期类型。Python `date` 必须先转换为约定格式，例如 ISO 8601 的 `YYYY-MM-DD` 字符串。

### 问题二：为什么使用 `model_dump(mode="json")`，不用 `default=str`？

Pydantic JSON 模式根据模型字段进行可预测转换，Schema 边界清晰；`default=str` 会对所有未知对象兜底，容易把本应尽早暴露的类型错误隐藏起来。

### 问题三：为什么要同时测试报告初稿和报告修订？

两条路径都会把法规引用发送给模型。只修初稿会导致首次生成成功，但人工审批要求修改时再次出现同样故障，所以测试必须覆盖所有共享数据结构的消费端。

### 问题四：为什么以前的 82 项测试没有发现？

已有工作流测试主要覆盖离线模式和各周功能边界，没有构造“在线 LLM + 已审核引用 + 非空法规生效日期”的组合。这说明测试不仅要按模块划分，还要覆盖跨功能集成后新增的数据类型组合。

---

# 2026-08-01：费用统计从美元迁移为人民币

## 1. 解决的问题

监控面板最初固定显示美元 `$`，环境变量、接口字段和数据库列也使用 `USD` 后缀。当前实际使用的 `deepseek-v4-flash` 和 `text-embedding-v4` 服务商报价均为人民币，继续使用美元字段需要额外维护汇率，既不直观，也容易把人民币单价误填到美元变量中。

本次目标不是只把前端符号从 `$` 改成 `¥`，而是让配置、计算、持久化、接口和展示采用同一个人民币语义。

## 2. 修改或新增的关键文件

- `app/config.py`
  - 费用配置切换为 `LLM_INPUT_COST_PER_1M_CNY`、`LLM_OUTPUT_COST_PER_1M_CNY` 和 `EMBEDDING_COST_PER_1M_CNY`。
- `app/services/observability.py`
  - 费用计算函数改为 `estimate_cost_cny()`；
  - Span 记录字段改为 `estimated_cost_cny`。
- `app/models.py`、`app/migrations.py`
  - 运行与 Span 新增人民币费用列；
  - 旧美元列作为本地数据库兼容字段保留并固定写入 0。
- `app/schemas.py`、`app/main.py`、`app/services/monitoring.py`
  - 状态、单次监控和概览接口统一返回 CNY 字段。
- `frontend/src/types.ts`、`frontend/src/App.tsx`、`frontend/src/MonitoringPage.tsx`
  - TypeScript 类型同步；
  - 金额显示为 `¥0.000000`，并明确标注“人民币”。
- `.env`、`.env.example`
  - 本机按用户提供的报价配置输入 1 元、输出 2 元、Embedding 0.5 元，单位均为每百万 Token；
  - 示例文件继续默认 0，避免替其他使用者编造价格。
- `docs/openapi.json`、配置文档、接口文档、自测文档、数据库文档和求职材料
  - 同步字段、币种、测试数量与迁移说明。
...
```text
Chat 人民币费用
  = 输入 Token × 输入人民币单价 / 1,000,000
  + 输出 Token × 输出人民币单价 / 1,000,000

Embedding 人民币费用
  = 输入 Token × Embedding 人民币单价 / 1,000,000
```

币种必须贯穿字段命名。若数据库仍叫 `estimated_cost_usd`、接口却显示人民币，即使当前数值看起来正确，也会给后续开发、数据分析和财务核对制造语义错误。
...
.env 人民币/百万 Token 单价
  → Settings 读取 CNY 配置
  → Chat/Embedding Span 记录输入输出 Token
  → estimate_cost_cny 按 Span 计算人民币费用
  → agent_run_spans.estimated_cost_cny
  → 聚合到 agent_runs.estimated_cost_cny
  → monitoring API 返回 estimated_cost_cny
  → React 使用 ¥ 显示单次与累计费用
```

已有 SQLite 启动时通过增量迁移增加 CNY 列，不要求用户删除数据库。旧 USD 列不能直接重命名或复用，因为它保存的是另一种币种语义；兼容列继续写入 0，防止旧表上的 `NOT NULL` 约束阻止新增 Span。

## 5. 测试和实际结果

迁移初次测试真实发现旧 `agent_run_spans.estimated_cost_usd` 是 `NOT NULL` 列。若只删除 SQLAlchemy 模型中的旧字段，INSERT 会失败，Trace 变成空列表。修复方式是在 ORM 中保留旧兼容字段、写入 0，同时所有新计算只使用 CNY 列。

最终结果：

- 人民币费用计算测试通过；
- 旧 SQLite 表增量迁移和 Span 写入通过；
- 完整 pytest：84 / 84 通过；
- Ruff check：通过；
- Mypy：27 个检查范围内的源文件无错误；
- TypeScript 与 Vite 生产构建：通过，190 个模块完成构建；
- OpenAPI JSON：成功重新生成并通过解析校验。

## 6. 已知限制

1. 历史运行不会根据人民币单价自动重算，只有修改配置后产生的新 Span 才有人民币费用；
2. DeepSeek 缓存命中与未命中的输入价格不同，当前 Usage 只统计总输入 Token，因此按未命中价格进行偏保守估算；
3. 免费额度、批量折扣、服务商账单舍入和套餐优惠尚未计入；
4. 本次保留旧 USD 列是 SQLite 兼容策略，正式数据库迁移可在备份和校验后删除废弃列；
5. 费用仍用于工程估算，不是财务对账结果。

## 7. 面试可能追问与回答要点

### 问题一：为什么不能只修改前端货币符号？

只改符号会让数据库和 API 中名为 USD 的数值被当作人民币解释，形成静默的数据语义错误。币种属于数据契约，必须同步配置、计算、字段、接口和展示。

### 问题二：为什么不把历史美元列直接改名？

改名不能完成币种换算，而且旧数据可能是按美元单价计算的。新增 CNY 列能明确区分新旧语义；历史数据是否换算需要独立、可审计的回填任务。

### 问题三：为什么还保留美元列？

用户已有 SQLite 表的旧列带 `NOT NULL` 约束。直接从 ORM 删除字段会导致新记录 INSERT 失败。兼容期内固定写 0，业务接口只暴露 CNY；未来使用正式迁移工具时再删除旧列。

### 问题四：缓存命中价格怎么处理？

当前选择未命中单价作为保守估计。更准确的方案是从服务商 Usage 中分别采集缓存命中和未命中 Token，为两类输入配置不同单价，再分别计算并展示。

---

# 2026-08-01：修复轮询清空人工审批意见

## 1. 解决的问题

报告进入 `waiting_for_approval` 后，用户在“审批意见”文本框输入内容，最多约 1.5 秒就会被自动清空，导致“要求修改”和“驳回报告”无法稳定提交。

根因不是审批接口或数据库，而是前端每 1.5 秒轮询任务状态。每次轮询到同一个审批待办时，代码都会重新加载草稿、引用和版本，并无条件执行：

```tsx
setApprovalComment("");
```

`textarea` 是由 `approvalComment` 控制的受控组件，所以状态被清空会立即覆盖用户正在编辑的 DOM 内容。

## 2. 修改或新增的关键文件

- `frontend/src/App.tsx`
  - 新增 `approvalActionIdRef` 记录当前审批待办 ID；
  - 只有检测到新的 `action_id` 时才初始化为空意见；
  - 同一个审批待办的后续轮询只刷新服务器状态，不修改本地输入；
  - 创建新任务、重置页面或审批成功后明确清空旧意见。
- `SELF_TEST.md`
  - 增加“等待至少两轮轮询、输入仍保留”的人工审批回归步骤；
  - 记录真实浏览器验收结果。

## 3. 核心实现原理

服务器状态和用户正在编辑的表单状态具有不同生命周期：

- 服务器状态：任务进度、报告版本、待办动作，需要轮询刷新；
- 本地草稿状态：审批人尚未提交的文字，不应被服务器刷新覆盖。

审批 `action_id` 是待办身份。只有身份变化才代表进入新的审批轮次：
...
使用 `useRef` 是因为它能跨渲染保存身份，但更新时不会触发额外渲染。真正需要展示的审批意见继续放在 `useState` 中。

## 4. 请求和数据流转

```text
Agent 进入 waiting_for_approval
  → 前端 GET /runs/{id}
  → GET /runs/{id}/pending-action
  → 首次看到 approval action_id
  → 清空上一轮意见并展示审批卡
  → 用户输入审批意见
  → 后续每 1.5 秒轮询相同 action_id
  → 只更新任务/报告数据，保留 approvalComment
  → 用户提交批准、修改或驳回
  → POST /runs/{id}/approvals
  → 成功后清空意见并退出当前待办
```

如果用户要求修改，Agent 生成新报告版本后会创建新的审批 `action_id`。前端检测到身份变化，再为新一轮审批准备空白意见。
...
1. 使用独立临时 SQLite 和离线后端创建合同纠纷任务；
2. 任务成功进入 `waiting_for_approval`；
3. 输入“请补充设备逾期交付造成三万元差价损失的证据，并明确解除合同通知到达时间。”；
4. 经历至少两次 1.5 秒轮询后，DOM 中的完整文本仍然存在；
5. 浏览器控制台无 error/warning；
6. 验收后停止临时后端并删除临时数据库、WAL/SHM 和日志文件。

## 6. 已知限制

1. 审批意见只保存在当前 React 内存中，整个页面强制刷新或关闭浏览器后尚不能恢复；
2. 多浏览器标签页不会实时同步各自尚未提交的意见；
3. 当前等待审批时仍持续轮询，虽然请求量很小，但可以优化为待办加载完成后暂停、提交后恢复；
4. 项目尚未引入前端单元测试框架，本次使用 TypeScript 构建和真实浏览器多轮轮询验收覆盖该交互 Bug。

## 7. 面试可能追问与回答要点

### 问题一：为什么轮询会清空输入？

轮询本身不会清空输入，是轮询回调把服务器数据和本地编辑状态混在一起处理，无条件重置了受控组件的 state。

### 问题二：为什么使用 action_id，而不是判断状态字符串？

多个报告版本都会处于 `waiting_for_approval`，状态字符串无法区分审批轮次。`action_id` 是具体待办的稳定身份，只有它变化才应初始化新表单。

### 问题三：为什么不用变量保存 action_id？

函数组件每次渲染都会重新执行，普通局部变量无法跨渲染保留。`useRef` 可以保存身份且不引发额外渲染，适合这种非展示型状态。

### 问题四：如何进一步支持刷新后恢复审批草稿？

可以按 `run_id + action_id` 把未提交意见保存到 `sessionStorage`，或增加后端草稿接口。保存前要明确数据保留、隐私和多标签冲突策略。

---

# 2026-08-01：数据库逐表字段文档与数据量分析

## 1. 完成内容

将原本只介绍数据库文件和少量核心表的 `db.md`，重构为覆盖当前真实 SQLite 的完整学习文档。文档说明 14 张表、所有字段、主外键关系、数据流、当前行数、常用只读 SQL，以及 `agent_run_citations`、Embedding、Span、checkpoint 和 writes 行数较多的原因。

## 2. 解决的问题

PyCharm 截图仍显示 7 张表，容易让学习者误以为当前项目只有早期表结构；同时 `agent_run_citations` 有 1180 行，容易被误解成重复插入或知识库中有 1180 条法律。

真实检查结果：

- 当前数据库有 14 张项目/运行时表；
- 287 次 Agent 运行中有 236 次到达检索阶段；
- 每次保存 Top-5 引用，因此 `236 × 5 = 1180`；
- 所有有引用的运行都恰好为 5 条；
- `(run_id, article_id)` 重复组为 0；
- 1139 条引用通过审核，41 条为低置信；
- 51 次没有引用的运行由 22 次等待追问、19 次排队和 10 次提前失败构成。
...
判断数据库“数据为什么多”，必须先确认一行的业务粒度：

```text
agent_runs：一行 = 一次 Agent 运行
agent_run_citations：一行 = 一次运行的一条候选法条
article_embeddings：一行 = 一条法条的一套 provider/model 向量
agent_run_spans：一行 = 一个节点或模型调用
checkpoints：一行 = 一个工作流状态快照
writes：一行 = 一个 checkpoint 的一次通道写入
```

一对多链路天然会放大行数。Top-K=5 表示每次运行最多产生 5 条引用快照；两套 Embedding 表示每条法规产生两条向量记录；一个 Agent 运行有多个节点、恢复和模型调用，因此有多个 Span；一个 checkpoint 又会写多个状态 channel。

## 5. 数据流

```text
laws → law_versions → legal_articles → article_embeddings
                                      ↓
agent_runs → agent_run_citations → report_versions → agent_approvals
...
通过 SQLAlchemy Inspector 逐表读取真实表名、列、类型、可空性、外键和索引，并查询实际行数与分组统计。文档完成后使用脚本逐列检查：

- 14 张表全部在文档中出现；
- 所有数据库字段均能在 `db.md` 中找到；
- 缺失表/字段列表为空；
- Markdown 代码围栏数量为偶数；
- `git diff --check` 在清理行尾空格后通过。

## 7. 已知限制

1. 文档中的行数是 2026-08-01 本机快照，继续运行项目后会变化；
2. `checkpoints` 和 `writes` 是 LangGraph 内部实现，未来升级依赖后字段可能变化；
3. 当前文档没有生成 ERD 图片，只提供 Mermaid，取决于阅读器是否支持渲染；
4. 正式库已有开发和测试痕迹，若要清理应先备份并设计按运行范围删除的脚本，不能直接手工删子表；
5. PyCharm 显示的表数可能是结构缓存，刷新前不能把界面数量当作真实 Schema。

## 8. 面试可能追问与回答要点

### 问题一：为什么不直接把引用存在报告 JSON 里？

独立引用表便于按运行、法条、版本、分数和审核状态查询，也能建立唯一约束和索引；报告版本仍保存引用快照，用于保证历史报告不可变。

### 问题二：1180 条引用是不是数据重复？

不是。236 次运行分别保存 Top-5，所以正好 1180；同一次运行与同一法条的组合没有重复。跨运行引用同一法条是正常的，因为每次案件的查询、分数、法规时点和审核结果不同。

### 问题三：为什么同一法条要保存两套向量？

`(article_id, provider, model)` 唯一键允许 Hash 384 维和百炼 `text-embedding-v4` 1024 维并存。离线演示与在线语义检索可以切换，又不会覆盖或混算不同维度。

### 问题四：为什么 writes 比 checkpoints 多很多？

checkpoint 是一次状态快照，而每个快照包含多个 channel 写入，例如 facts、citations、traces、report 和 approval。它们是“一对多”关系，因此 writes 通常是数据库中行数最多的表。
